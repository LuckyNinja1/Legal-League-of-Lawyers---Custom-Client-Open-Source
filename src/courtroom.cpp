...
Courtroom::Courtroom(AOApplication *p_ao_app) : QMainWindow()
{
  ...
  lightbulb_timer = new QTimer(this);
  lightbulb_timer->setSingleShot(true);

  whack_timer = new QTimer(this);
  whack_timer->setSingleShot(true);

  whoops_timer = new QTimer(this);
  whoops_timer->setSingleShot(true);
  ...
  ui_vp_lightbulb = new AOImage(this, ao_app);
  ui_vp_whack = new AOImage(this, ao_app);
  ui_vp_whoops = new AOImage(this, ao_app);
  ...
  ui_lightbulb = new AOButton(this, ao_app);
  ui_whack = new AOButton(this, ao_app);
  ui_whoops = new AOButton(this, ao_app);
  ...
  connect(lightbulb_timer, SIGNAL(timeout()), this, SLOT(lightbulb_done()));
  connect(whack_timer, SIGNAL(timeout()), this, SLOT(whack_done()));
  connect(whoops_timer, SIGNAL(timeout()), this, SLOT(whoops_done()));
  ...
  // connect LLL realization buttons
  connect(ui_lightbulb, SIGNAL(clicked()), this, SLOT(on_lightbulb_clicked()));
  connect(ui_whack, SIGNAL(clicked()), this, SLOT(on_whack_clicked()));
  connect(ui_whoops, SIGNAL(clicked()), this, SLOT(on_whoops_clicked()));
...
void Courtroom::set_widgets()
{
  ...
  if (ao_app->lll_ic_support_enabled)
  {
      ui_lightbulb->show();
      ui_whack->show();
      ui_whoops->show();
  ...
  ui_vp_lightbulb->move(ui_viewport->x(), ui_viewport->y());
  ui_vp_lightbulb->resize(ui_viewport->width(), ui_viewport->height());
  ui_vp_lightbulb->set_image("lightbulbflash.png");
  ui_vp_lightbulb->hide();

  ui_vp_whack->move(ui_viewport->x(), ui_viewport->y());
  ui_vp_whack->resize(ui_viewport->width(), ui_viewport->height());
  ui_vp_whack->set_image("whackflash.png");
  ui_vp_whack->hide();

  ui_vp_whoops->move(ui_viewport->x(), ui_viewport->y());
  ui_vp_whoops->resize(ui_viewport->width(), ui_viewport->height());
  ui_vp_whoops->set_image("whoopsflash.png");
  ui_vp_whoops->hide();
  ...
  set_size_and_pos(ui_lightbulb, "lightbulb");
  ui_lightbulb->set_image("lightbulb.png");

  set_size_and_pos(ui_whack, "whack");
  ui_whack->set_image("whack.png");

  set_size_and_pos(ui_whoops, "whoops");
  ui_whoops->set_image("whoops.png");
...
void Courtroom::list_areas()
{
  ...
  QBrush precasing_brush(ao_app->get_color("area_pre_casing_color", f_file));
  QBrush casing_brush(ao_app->get_color("area_casing_color", f_file));
  QBrush postcasing_brush(ao_app->get_color("area_post_casing_color", f_file));
  ...
  QBrush mod_locked_brush(ao_app->get_color("area_mod_locked_color", f_file));
  ...
  for (int n_area = 0 ; n_area < area_list.size() ; ++n_area)
  {
    ...
    if (i_area.toLower().contains(ui_music_search->text().toLower()))
    {
      ...
      if (ao_app->arup_enabled)
      {
        ...
        else if (arup_locks.at(n_area) == "MOD")
        {
            ui_area_list->item(n_listed_areas)->setBackground(mod_locked_brush);
        }
        else
        {
            if (arup_statuses.at(n_area) == "LOOKING-FOR-PLAYERS")
                ui_area_list->item(n_listed_areas)->setBackground(lfp_brush);
            else if (arup_statuses.at(n_area) == "PRE-CASING")
                ui_area_list->item(n_listed_areas)->setBackground(precasing_brush);
            else if (arup_statuses.at(n_area) == "CASING")
                ui_area_list->item(n_listed_areas)->setBackground(casing_brush);
            else if (arup_statuses.at(n_area) == "POST-CASING")
                ui_area_list->item(n_listed_areas)->setBackground(postcasing_brush);
...
// In LLL's Case (NOTE: NOT ALL FEATURES LISTED HERE ARE IN YET!!! DON'T EXPECT TO SEE PERCIVE IF IT'S NOT ADDED):
  //psychelocks_enabled#
  //psychelocks_number# -> This is the number of psyche-locks that are being used.
  //psychelocks_color# -> This is the color of the psyche-locks. The only options generally will be red and black, but I might allow it to be customized.
  //perceive_enabled#
  //perceive_chunks# -> This is how many words are in each chunck. This will be a bit harder to create since I will need characters to have twitchy/fidgity animations.
  //moodmatrix_enabled#
  //moodmatrix_selection#% -> This is the selection the defense/prosecution (if athena pro???) will push on.
// LLL's Unadded List
  //psychelocks
  //perceive
  //moodmatrix
...
void Courtroom::lightbulb_done()
{
  ui_vp_lightbulb->hide();
}

void Courtroom::whack_done()
{
  ui_vp_whack->hide();
}

void Courtroom::whoops_done()
{
  ui_vp_whoops->hide();
}
...
void Courtroom::start_chat_ticking()
{
  ...
  int realization_mod = m_chatmessage[REALIZATION].toInt();
  switch (realization_mod)
  {
    case 1:
    {
    ...
    case 2:
    {
      lightbulb_timer->start(60);
      ui_vp_lightbulb->show();
      sfx_player->play(ao_app->get_custom_lightbulb(m_chatmessage[CHAR_NAME]));
      break;
    }
    case 3:
    {
      whack_timer->start(60);
      ui_vp_whack->show();
      sfx_player->play(ao_app->get_custom_whack(m_chatmessage[CHAR_NAME]));
      break;
    }

    case 4:
    {
      whoops_timer->start(60);
      ui_vp_whoops->show();
      sfx_player->play(ao_app->get_custom_whoops(m_chatmessage[CHAR_NAME]));
      break;
    }
    case 0:
    {
    ...
    default:
    {
       if (realization_mod > 4 || realization_mod < 0 || realization_mod == NULL)
       {
        qDebug("Error in realization logic!");
       }
    }
  }
  ui_vp_message->clear();
...
void Courtroom::on_ooc_return_pressed()
{
  ...
  else if (ooc_message.startsWith("/login") || ooc_message.startsWith("/mod"))
  {
    ui_guard->show();
  ...
  else if (ooc_message.startsWith("/rainbow") && ao_app->yellow_text_enabled && !rainbow_appended)
  {
    ...
    append_server_chatmessage("CLIENT", tr("Why are you trying to get Rainbow text? It's already in you dunce."), "1");
    return;
  }
  ...
  else if (ooc_message.startsWith("/enable_blocks_ccc"))
  {
  ...
  else if (ooc_message.startsWith("/enable_blocks_lll"))
  {
      append_server_chatmessage("CLIENT", tr("You have forcefully enabled features that the server may not support. You may not be able to talk in IC, or worse, because of this."), "1");
  }
  ...
  else if (ooc_message.startsWith("/find_missing_music") || ooc_message.startsWith("/find_missing_songs"))
  {
      QFile file("missing_songs_list.txt");
      if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate))
      {
          append_server_chatmessage("CLIENT", tr("The missing music debug log is missing. Just create a text document called \"missing_songs_list.txt\" in your base folder.\nNote: The file name is case-sensitive."), "1");
          ui_ooc_chat_message->clear();
          return;
      }
      QTextStream out(&file);
      ...
      if (music_list.size() > 0)
      {
        for (int n_song = 0; n_song < music_list.size(); ++n_song)
        {
            ...
            append_server_chatmessage("CLIENT", tr("All missing music has been saved in \"missing_songs_list.txt\""), "1");
            ui_ooc_chat_message->clear();
            return;
        }
      }
  }
  else if (ooc_message.startsWith("/show_judge_buttons"))
  {
    toggle_judge_buttons(true);
  }
  else if (ooc_message.startsWith("/hide_judge_buttons"))
  {
    toggle_judge_buttons(false);
  }
  ...
  else if(ooc_message.startsWith("/save_case"))
  {
      ...
      for(int i = local_evidence_list.size() - 1; i >= 0; i--)
      {
           ...
           casefile.setValue("owner",local_evidence_list[i].owner);
           casefile.endGroup();
      }
      casefile.sync();
...
void Courtroom::on_lightbulb_clicked()
{
  if (realization_state != 2)
  {
    realization_state = 2;
    ui_realization->set_image("realization.png");
    ui_lightbulb->set_image("lightbulb_pressed.png");
    ui_whack->set_image("whack.png");
    ui_whoops->set_image("whoops.png");
  }
...
void Courtroom::on_whack_clicked()
{
  if (realization_state != 3)
  {
    realization_state = 3;
    ui_realization->set_image("realization.png");
    ui_lightbulb->set_image("lightbulb.png");
    ui_whack->set_image("whack_pressed.png");
    ui_whoops->set_image("whoops.png");
  }
...
void Courtroom::on_whoops_clicked()
{
  if (realization_state != 4)
  {
    realization_state = 4;
    ui_realization->set_image("realization.png");
    ui_lightbulb->set_image("lightbulb.png");
    ui_whack->set_image("whack.png");
    ui_whoops->set_image("whoops_pressed.png");
  }
